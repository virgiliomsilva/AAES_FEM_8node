...
Original file is located at
    https://colab.research.google.com/drive/1NhdtFlEZnNXJ-Jqt8Lc0b-ftdwlDmGTS
"""

#### notas
# patch test
#criar função dNds ###################################
#por as funções no inicio de tudo
...
print('descriç do programa .quadrilátero.. plase stress isoparametrico , esp v e E constantes,uma força concentrada, uma força distribuida cosntante')
print('Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been')
print()
print('Carregue no Enter para continuar...')
input()
print('informações a dar ao utilizador e a imagem de como deve preencher .. industrys standard dummy text ever since the 1500s, when an ')
print('unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, ')
print('but also the leap into electronic typesetting, remaining essentially unchanged.')
print()
print('Carregue no Enter para continuar...')
input()
...
# Material Properties
E = float(input('Módulo de Young (GPa): ')) * 10 ** 6
...
for j in range(8):
    for i in range(2):
        if i == 0:
            xbh[[i],[j]] = float(input('Coordenada X do ponto ' + str(j + 1) + ' (m): '))            
        else:
            xbh[[i],[j]] = float(input('Coordenada Y do ponto ' + str(j + 1) + ' (m): '))

xbv = np.transpose(xbh)
e = float(input('Espessura do elemento (m): '))
...
if load_type == str('d'):
    Qpx = float(input('Coordenada X do ponto de aplicação (m): '))
    Qpy = float(input('Coordenada Y do ponto de aplicação (m): '))
    Qvx = float(input('Valor da carga na direção X (kN):'))
    Qvy = float(input('Valor da carga na direção Y (kN):'))
    ...
    bt = float(input('Componente tangencial da carga distribuída(kN): '))
    #bt2 = float(input('Componente tangencial da força distribuída(kN): '))
    #bt3 = float(input('Componente tangencial da força distribuída(kN): '))
    print('Componente normal positiva para o interior do elemento')
    bn = float(input('Componente normal da carga distribuída (kN): '))
    #bn2 = float(input('Componente normal da força distribuída (kN): '))
    #bn3 = float(input('Componente normal da força distribuída (kN): '))
